...
struct ImmersiveView2: View {
    var body: some View {
        RealityView { content, attachments in
            guard let rootEntity = try? await Entity(named: "Scene2", in: realityKitContentBundle) else { return }
...
//                controlView.components.set(inputComponent)
    ...
    //                let animationLibrary = entity.components[AnimationLibraryComponent.self]
                    if let animationResource = entityHalfLeft.availableAnimations.first {
                        entityHalfLeft.playAnimation(animationResource.repeat(count: 500), transitionDuration: 2, startsPaused: false)
                    }
                }
